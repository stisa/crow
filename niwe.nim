import webgl, dom,sequtils, math
...
type 
  GL = WebglRenderingContext # shorthand
...
converter extractProgram(program: Program): WebglProgram = program.program
converter toF32A(m:Matrix):Float32Array = {.emit: "`result` = new Float32Array(`m`);\n".}

proc getContextFromId*(id:string,setViewport:bool=false):GL =
  ## Get a context from a canvas with id `id`
  let canvas = document.getElementById(id).Canvas
  result = getContextWebgl(canvas)
  if setViewport: result.viewport(0,0,canvas.width, canvas.height)
...
proc initShader(gl:GL, typ:ShaderType, src:string):WebGLShader =
  ...
  if not gl.getStatus(result): log gl.getShaderInfoLog(result)
  #else: log "Shader compiled ok"

proc initProgram(gl:GL, useIt:bool=false ,vertex_src:string=vsrc, fragment_src:string=fsrc):Program =
  ## Init, compile and link a program.
  ## If `useIt` is true, also sets the program as current
  var program = gl.createProgram()
  let vs = gl.initShader(Vertex, vmsrc)
  let fs = gl.initShader(Fragment, fsrc)
